...
----
+ 
The production maven profile tiles Vaadin to package all the required node dependencies without the final jar.
...
. Test your application by navigating to the root URL of the application `/appui`, which will invoke Vaadin UI.  You should now see a table listing the first set of rows returned from the cities microservice:
+
image::images/ui.jpg[]
